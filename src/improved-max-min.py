...
# Improved Max Min Algorithm |
# ---------------------------+
# Select task ==> Maximum execution time
# Assign machine ==> Minimum completion time
...
while task_queue:
    ...
    filtered_execution_time = dict(filter(lambda x: x[0] in task_queue, execution_time.items()))
    
    # selected_task ==> task_max_execution_time
    selected_task = max(filtered_execution_time.items(), key=lambda x: max(x[1].values()))[0]
    print(f"selected_task {selected_task}")
    
    # selected_machine ==> machine_min_task_completion_time
    selected_machine = min(completion_time[selected_task].items(), key=lambda x: x[1])[0]
    print(f"selected_machine {selected_machine}")

    # task "task_max_execution_time" assigned to machine "machine_min_task_completion_time"
...
selected_task t3
selected_machine m2
Task t3 assigned to m2
Ready time for m2 updated to 1
------
Task Queue ['t4']
selected_task t4
selected_machine m1
